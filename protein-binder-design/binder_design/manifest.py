...
agent-toolkit ``protein-binder-design`` skill. A campaign manifest is a single
JSON file recording every candidate's lineage, scores, artifacts, and filter
status -- the backbone for ranking, resumability, validation, and the report.
"""
...
class Manifest:
    ...
    @classmethod
    def create(cls, run_dir, target, mode="hosted", params=None, filters=None) -> "Manifest":
    ...
    def log_stage(self, name, **info) -> None:
    ...
    def upsert_candidate(self, cid, **fields):
        c = self._find(cid)
        if c is None:
            c = {
                "id": cid,
    ...
    def rank(self, by="iptm", descending=True, passed_only=False, include_controls=False):
    ...
    def to_csv(self, path=None) -> Path:
        ...
        with open(path, "w", newline="") as fh:
            ...
            for c in self.data["candidates"]:
                row = [
                    c.get("id"), c.get("backbone_id"), c.get("is_control"),
                    c.get("control_type"), c.get("passed_filter"),
                ]
