...
agent-toolkit ``protein-binder-design`` skill.
"""
...
def ca_rmsd(coords_a, coords_b):
    """CA-RMSD between two coordinate lists; truncates to the common length."""
    n = min(len(coords_a), len(coords_b))
    if n == 0:
        raise ValueError("no CA atoms found for RMSD")
    return kabsch_rmsd(coords_a[:n], coords_b[:n])
